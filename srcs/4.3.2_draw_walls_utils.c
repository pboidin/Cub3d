...
int		ft_bit_mapper(t_ray *ray, int bitmap_wi) // En se basant sur l'endroit exact où le rayon frappe le mur, nous pouvons trouver quelle partie de la texture à utiliser pour le mur
{
	...
	if (ray->side == 'H') // //Savoir si ce rayon est horizontal ou vertical nous permet de savoir quelle coordonnée de rayon aura une partie fractionnaire car s'il s'agit d'un mur, les coordonnées sont nécessairement du format format x:1.00/y:2.42 ou x:1.42/y:2.00
	{
...
void	ft_draw_strip_w(t_rect *rect, int *img, t_reso *reso, t_ray *ray)
{
	...
	while (++y < rect->hei && y < reso->height)
	{
		...
		img[(rect->y * reso->width) + (y * reso->width)
			+ rect->x] = rect->tex.img_tex.data[y_text * rect->tex.height + x_text];
	}
}